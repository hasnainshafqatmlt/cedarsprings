...
try {
    ...
    $logger->error("Unable to create a new account in ajax/putCreateAccount: " .$e->getMessage());
...
if(!is_array($response)) {
    ...
    $logger->error('Ultracamp did not respond to the create account API request.');
...
if(!isset($user['Authenticated']) || $user['Authenticated'] !== true) {
    ...
    $logger->error("Unable to log into newly created account " .$newAccount['IdAccount'] ." with email address ". $accountInfo['email']);
    return true;
}
...
if(strtolower($accountInfo['email'] != strtolower($newAccount['UserName']))) {
    $logger->d_bug("User name modification took place for " .$accountInfo['email'], $newAccount);
