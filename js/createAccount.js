  ...
  $.ajax({
    ...
    .done(function (data) {
      ...
      if (data["Authenticated"] == true) {
        ...
        //document.location = "addPerson" +get;

        document.location = "/camps/queue/addperson/";
        return true;
      }
      ...
      if (Array.isArray(data) && data[0] === "User name already exists") {
        $(".ultracamp_error").html(
          'There is already an account with this email address. Please try to recover your existing account by <a href=# data-toggle="modal" data-target="#myModal" data-msg="resetAccount">clicking here</a>.'
        );
...
function validateForm() {
      ...
      case "phone":
        ...
        if (!phoneRegex.test(value.replace(/[^\d]/g, ""))) {
          ...
          formGroup
            .find(".new-account-input")
            .append(
              '<div class="field-error">Please enter a valid 10-digit phone number</div>'
            );
          $(".field-error").show();
        }
        break;
      ...
      case "zip":
        ...
        if (!zipRegex.test(value)) {
          ...
          formGroup
            .find(".new-account-input")
            .append(
              '<div class="field-error">Please enter a valid 5-digit zip code</div>'
            );
          $(".field-error").show();
        }
        break;
  ...
  if (!passwordRegex.test(password)) {
    ...
    passwordGroup
      .find(".new-account-input")
      .append(
        '<div class="field-error">Password must be at least 7 characters and contain both letters and numbers</div>'
      );
    $(".field-error").show();
  }
